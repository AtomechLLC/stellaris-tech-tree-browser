...
**Domain:** Interactive game-data visualization web app (static/client-side, build-time data pipeline)
**Researched:** 2026-07-07
**Confidence:** HIGH (core choices verified against npm registry + official docs; MEDIUM/LOW flagged inline where only community sources exist)
...
```bash
# Core app
npm install react react-dom sigma graphology @react-sigma/core

# Layout
npm install elkjs graphology-layout

# Search & state
npm install fuse.js zustand

# Build-time data pipeline (devDependencies — never shipped to client)
npm install -D jomini sharp

# Tooling
npm install -D vite @vitejs/plugin-react typescript vitest eslint prettier
...
**If icon fidelity/color-accuracy turns out to matter a lot (Paradox may use non-standard DXT compression variants):**
...
**If the tech tree needs runtime graph algorithms (e.g., "show me the shortest prerequisite path to tech X"):**
...
**If future scope grows to include mod support (explicitly out of scope per PROJECT.md, but noted for future-proofing):**
...
---
*Stack research for: Stellaris Tech Tree Visualizer (build-time parsed static web app)*
*Researched: 2026-07-07*
